...
exports.createBootcamp = async (req, res) => {
  try {
    const { title, description, technologies, maxStudents, maxFaculty, startDate, endDate, location } = req.body;
    const bootcamp = await Bootcamp.create({
      title, description, technologies, maxStudents, maxFaculty, startDate, endDate,
      location, createdBy: req.user.id
    });

    res.status(201).json({ success: true, message: "Bootcamp created", data: bootcamp });
  } catch (err) {
...
exports.uploadResume = async (req, res) => {
  try {
    ...
    res.status(200).json({ success: true, message: "Resume uploaded", data: resume });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
};
...
exports.handleFacultySelection = async (req, res) => {
  try {
    ...
    // Notification logic
    const notifType = status === "accepted" ? "faculty_selected" : "faculty_rejected";
    ...
    res.status(200).json({ success: true, message: "Faculty selection updated" });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
};
